...
func TestJoinCAPin(t *testing.T) {
	...
	lock, err := types.NewLock("test-join-ca-pin-unrelated-lock", types.LockSpecV2{
		Target: types.LockTarget{User: "definitely-not-a-real-user"},
	})
	require.NoError(t, err)
	require.NoError(t, testSrv.Auth().UpsertLock(ctx, lock))
	require.Eventually(t, func() bool {
		return trace.IsAccessDenied(testSrv.Auth().CheckLockInForce(constants.LockingModeStrict, []types.LockTarget{{User: "definitely-not-a-real-user"}}))
