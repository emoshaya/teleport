...
| [aws_iam_policy_document.teleport_discovery_resource_enrollment](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) | data source |
...
| [aws_iam_policy_document.teleport_organization_discovery](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) | data source |
...
| aws\_iam\_role\_name\_for\_child\_accounts | Name for the AWS IAM role to assume in organization accounts. This role must be created manually in each child account. Check the module outputs for the trust relationship and permissions required. | `string` | `"teleport-discovery-for-child-account"` | no |
...
| enroll\_organization\_accounts | Discover resources in all the AWS accounts under the organization. Only supported for EC2 resource discovery and requires manually creating an AWS IAM role in each child account. Check the module outputs for the trust relationship and permissions required. | `bool` | `false` | no |
...
| iam\_role\_to\_create\_in\_child\_accounts | Manually create the following AWS IAM Role in each AWS account under the organization. |
