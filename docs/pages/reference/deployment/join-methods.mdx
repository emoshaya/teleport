...
<Admonition type="note" title="See Also">
- [Deploying Machine & Workload Identity with Bound Keypair joining](../machine-workload-identity/bound-keypair/getting-started.mdx)
...
Teleport automatically detects the cloud from Azure IMDS (`compute.azEnvironment`)
and requests the matching Azure Resource Manager audience. You can optionally
override the cloud in agent configuration:
...
Supported `cloud_environment` values are:
`AzurePublicCloud`, `AzureChinaCloud`, `AzureUSGovernment`,
`AzureUSGovernmentCloud`, and `AzureGermanCloud`.
...
The Env0 join method is available to workloads running in the Env0 cloud
environment. The "Enable OIDC during deployments" option must be enabled for
tokens to be issued into runner environments.

This join method is typically used by the Teleport Terraform provider to
authenticate to Teleport from the Env0 cloud environment. It cannot be used to
join Terraform runs on other platforms and dedicated join methods should be used
instead.

The `env0` join method requires Teleport v18.4.0 or later.
...
utilizing Teleport Machine & Workload Identity and GitHub Actions.
