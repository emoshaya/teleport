...
2. Export the Teleport certificate authority by running the following command,
   assigning <Var name="teleport.example.com"/> to your Teleport cluster
   address:
   ...
   has multiple CA certificates (one for each Auth Service instance). You can download
   a bundle containing all CAs by appending `&format=zip` to the URL.
...
1. Create a `windows-desktop-admins.yaml` file to define a new role:
   ...
   to create local Windows logins, see [Automatic User Creation](reference/user-creation.mdx).
...
Getting Started](../../zero-trust-access/rbac-get-started/role-demo.mdx).
