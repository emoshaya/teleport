...
through an external identity provider, see [Configure Single Sign-on](../sso/sso.mdx).
...
- [Proxy peering](reliability/proxy-peering.mdx) must be disabled. Proxy Peering and Trusted Clusters are not compatible.
...
- The clusters `east` and `west` are leaf clusters that trust a self-hosted Teleport
  Cluster, for example, `teleport.example.com`.
- Users always authenticate against `teleport.example.com` but use their
