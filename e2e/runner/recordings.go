...
// recordingInstance represents a recording assigned to a specific user.
// When a recording is shared by multiple users, each gets its own instance
// with a unique destSessionID (the original ID with a numeric suffix for copies).
type recordingInstance struct {
	recording
	user          string
	destSessionID string // the session ID written to disk; differs from recording.SessionID for copies
}
...
// When a recording is owned by multiple users, it is duplicated with a unique session ID per user.
func seedRecordings(ctx context.Context, e2eDir, dataDir string, recordingOwners map[string][]string) error {
	if len(recordingOwners) == 0 {
		return nil
	}
	...
	for recID, users := range recordingOwners {
		rec, ok := byID[recID]
		if !ok {
		...
		for i, user := range users {
			sid := rec.SessionID
			if i > 0 {
				sid = fmt.Sprintf("%s-%d", rec.SessionID, i+1)
			}
	...
	for _, inst := range instances {
		for _, ext := range []string{".tar", ".metadata", ".thumbnail"} {
			src := filepath.Join(filepath.Dir(inst.Path), inst.recording.SessionID+ext)
	...
	slog.Info("seeded session recordings", "recordings", len(recordingOwners), "instances", len(instances))
...
func readAndPatchEvent(ctx context.Context, inst recordingInstance, newStop time.Time) (apievents.AuditEvent, error) {
	...
	for {
		...
		case *apievents.SessionEnd:
			...
			e.ServerMetadata.ServerID = inst.destSessionID
