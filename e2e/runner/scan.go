...
// This matches "user: {" but not "users: [" since the latter has 's' before ':'.
var userObjRe = regexp.MustCompile(`user:\s*\{`)
...
var traitsBlockRe = regexp.MustCompile(`traits:\s*\{`)

// traitKeyArrayRe matches a trait key followed by an array value (e.g. logins: ['root']).
var traitKeyArrayRe = regexp.MustCompile(`(\w+):\s*\[`)

// recordingsBlockRe matches the beginning of a recordings array declaration.
var recordingsBlockRe = regexp.MustCompile(`recordings:\s*\[`)

// loginAsBoolRe matches loginAs: true within a user object.
var loginAsBoolRe = regexp.MustCompile(`loginAs:\s*true`)

// defaultRoleNames are the built-in roles assigned to users when no roles are specified.
var defaultRoleNames = []scannedRole{
	{name: "access"},
	{name: "editor"},
}
...
func scanUsersFromTargets(targets []scanTarget) []scannedUser {
	var result []scannedUser
	for _, t := range targets {
		result = append(result, scanFileUsers(t.path, t.line)...)
	}
...
// user, users, and recordings are mutually exclusive — combining them is a fatal error.
func scanFileUsers(path string, targetLine int) []scannedUser {
	...
	for _, call := range findTestUseCalls(content) {
		...
		if count > 1 {
			slog.Error("user, users, and recordings are mutually exclusive in test.use()",
				"path", path,
				"line", callLine,
				"found", strings.Join(present, ", "),
			)
			os.Exit(1)
		}
