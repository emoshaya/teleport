...
type bootstrapResult struct {
	...
	// recordingOwners maps session recording IDs to the generated usernames
	// that should own them. A recording referenced by multiple users is
	// duplicated (one copy per user with a unique session ID).
...
func buildBootstrapState(e2eDir string, scannedUsers []scannedUser) (*bootstrapResult, error) {
	...
	recordingOwners := make(map[string][]string)
	...
	for _, su := range scannedUsers {
		...
		for _, rec := range su.recordings {
			recordingOwners[rec] = append(recordingOwners[rec], name)
		}
	}
