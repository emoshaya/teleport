...
// WithKubeServerScope is an option that sets the scope when building a KubernetesServerV3
func WithKubeServerScope(scope string) kubeServerOpt {
...
func NewKubernetesServerV3FromCluster(cluster *KubernetesClusterV3, hostname, hostID string) (*KubernetesServerV3, error) {
	return NewKubernetesServerV3(
		...
		WithKubeServerScope(cluster.GetScope()),
	)
}
