...
function wsRoute(target: string, path: string): [string, ProxyOptions] {
  const hostRewrite = shouldRewriteOrigin(target);
  ...
  return [
    path,
    {
      ...
      secure: false,
      ws: true,
      changeOrigin: hostRewrite,
      rewriteWsOrigin: hostRewrite,
    },
  ];
}
...
function shouldRewriteOrigin(target: string) {
  // Only rewrite Host / websocket Origin when vite is reached via a hostname that differs
  // from the upstream teleport proxy, otherwise Teleport sees an unknown host and 302s to /web/launch.

  return !!process.env.VITE_HOST && process.env.VITE_HOST !== target;
}
